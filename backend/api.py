...
async def health_check():
    ...
    # In LOCAL mode, skip database health check
    if config.ENV_MODE == EnvMode.LOCAL:
        logger.info("LOCAL mode: Skipping database health check")
        return {
            "status": "ok", 
...
if __name__ == "__main__":
    ...
    logger.info(f"Starting server on 0.0.0.0:8080 with {workers} workers")
    uvicorn.run(
        "api:app", 
        host="0.0.0.0", 
        port=8080,
