...
async def run_agent(
    ...
    # In local mode or if client is None, we'll use a default account ID
    from utils.config import config, EnvMode
    if config.ENV_MODE == EnvMode.LOCAL or client is None:
        logger.info(f"Local mode: Using default account ID for thread {thread_id}")
    ...
    if config.ENV_MODE == EnvMode.LOCAL or client is None:
        logger.info(f"Local mode: Using default sandbox info for project {project_id}")
        sandbox_info = {
            "id": "local-sandbox-123",
            "password": "local-sandbox-pass"
        }
    else:
    ...
    while continue_execution and iteration_count < max_iterations:
        ...
        if config.ENV_MODE == EnvMode.LOCAL or client is None:
            # In local mode, we'll assume the last message is from the user
            message_type = 'user'
            logger.info(f"Local mode: Assuming last message is from user for thread {thread_id}")
        else:
