...
def setup_api_keys() -> None:
    ...
    # Set up OpenAI API base for local LLM first
    if config.OPENAI_API_BASE:
        os.environ['OPENAI_API_BASE'] = config.OPENAI_API_BASE
        logger.info(f"Set OPENAI_API_BASE to {config.OPENAI_API_BASE}")
        
        # Set a dummy API key if none is provided
        if not config.OPENAI_API_KEY:
            os.environ['OPENAI_API_KEY'] = "sk-dummy-key"
            logger.info("Set dummy OPENAI_API_KEY for local LLM")
        else:
...
def prepare_params(
    ...
    # Handle local-mistral model
    if model_name == "local-mistral":
        logger.info("Using local Mistral model via llama.cpp server")
        model_name = "openai/mistral-7b-instruct"
        api_key = api_key or config.OPENAI_API_KEY or "sk-dummy-key"
        api_base = api_base or config.OPENAI_API_BASE or "http://localhost:8000/v1"
